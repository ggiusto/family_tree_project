# family_stories/views.py

from django.shortcuts import render, redirect, get_object_or_404
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from .models import FamilyStory
from .form import FamilyStoryForm

@login_required
def story_list(request):
    stories = FamilyStory.objects.all()
    context = {'stories': stories, 'title': 'Historias Familiares'}
    return render(request, 'family_stories/story_list.html', context)

@login_required
def story_create(request):
    if request.method == 'POST':
        form = FamilyStoryForm(request.POST)
        if form.is_valid():
        ...
        form = FamilyStoryForm()
    context = {'form': form, 'title': 'Añadir Nueva Historia'}
    return render(request, 'family_stories/story_form.html', context)

@login_required
def story_detail(request, pk):
    story = get_object_or_404(FamilyStory, pk=pk)
    context = {'story': story, 'title': f'Historia: {story.title}'}
    return render(request, 'family_stories/story_detail.html', context)
...
def story_update(request, pk):
    ...
    if request.method == 'POST':
        form = FamilyStoryForm(request.POST, instance=story)
        if form.is_valid():
            form.save()
            messages.success(request, 'Historia actualizada exitosamente.')
            return redirect('story_detail', pk=story.pk)
    else:
        form = FamilyStoryForm(instance=story)
...
def story_delete(request, pk):
    ...
    return render(request, 'family_stories/story_confirm_delete.html', context)
